var Hlsjs = require('hls.js').default;
var XhrAesLoader = require('hls.js/src/utils/xhr-aes-loader').default;
...
var registerSourceHandler = function (videojs) {
    ...
    function Html5Hlsjs(source, tech) {
        ...
        function _handleMediaError(error) {
            if (!_errorCounts[Hlsjs.ErrorTypes.MEDIA_ERROR] || _errorCounts[Hlsjs.ErrorTypes.MEDIA_ERROR] <= 1) {
        ...
        function switchQuality(qualityId) {
            if (_hls.config.levelSwitchStrategy && _hls.config.levelSwitchStrategy === 'instant') {
                _hls.currentLevel = qualityId;
            } else if (_hls.config.levelSwitchStrategy && _hls.config.levelSwitchStrategy === 'smooth') {
        ...
        function _onLevelLoad (event, data) { 
            ...
            if (typeof _dvrcheck === 'undefined' && data.details.live && _hls.streamController.lastCurrentTime >  (_hls.streamController.nextLoadPosition - _hls.streamController.lastCurrentTime) * 15) {
                // If we have before more than 10 chunks - its DVR (live stream usually has 3 chuncks in m3u8)
                tech.trigger('dvrsourcedetected')
            } else if(data.details.live && typeof _dvrcheck === 'undefined') {
        ...
        function _onMetaData(event, data) {
            ...
            _metadata = data;
           // But we can notify about qualitydata twice because "loadedqualitydata" mean that manifest was loaded. it must happen before play
        ...
        function _initHlsjs() {
            ...
            if (hlsjsConfig.aesSetup && hlsjsConfig.aesSetup.key && hlsjsConfig.aesSetup.iv) {
            ...
            _hls.on(Hlsjs.Events.LEVEL_SWITCHED, function (event, data) { tech.trigger("hlslevelswitched",data);});
